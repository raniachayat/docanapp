...
# Define actual mutation mapping per cancer type (from your data generation)
cancer_mutation_map = {
...
# Flat list of all mutations for fallback
all_mutations = list(encoders['mutation_1'].classes_)
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        # Select cancer type and dynamically update mutations outside form
        cancer_type_display = st.selectbox("Cancer Type", list(cancer_mutation_map.keys()))
        mutation_options = cancer_mutation_map.get(cancer_type_display, all_mutations)
        ...
        mutation_1 = st.selectbox("Mutation 1", mutation_options, key="mutation1")
        mutation_2 = st.selectbox("Mutation 2", ["None"] + mutation_options, key="mutation2")

        if st.button("Get Recommendation"):
            inputs = {
                'cancer_type': cancer_type_display,
                'age_group': age_group,
