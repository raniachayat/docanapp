...
def generate_pdf_report(name, pid, cancer, stage, age_grp, prev, mut1, mut2, recommended, advice_list, age_comment, prev_comment):
    ...
    pdf.ln(10)
    pdf.multi_cell(0, 10, f"Age Group Comment: {clean_text(age_comment)}")
    pdf.ln(1)
    pdf.multi_cell(0, 10, f"Previous Therapy Comment: {clean_text(prev_comment)}")
...
age_group_comments = {
    'Young': (
        "🧬 Younger patients (under_40 years) typically tolerate aggressive regimens better. "
    ...
    'Elderly': (
        "🧓 In patients more than 65, especially with ECOG ≥2, consider geriatric assessment. "
